                ...
                for inventive in Database.get_user_Inventives(reciver["email"]):
                    if inventive["emailsent"]==msg["email"] and inventive["typeinventive"]=="newchat":
                        if msg["type"]=="newchatagree":
                            ...
                            await send_WS_msg(sender["email"],{"type":"addchat","inv_id":inventive["id"],
                                                               "chat":{"id":id["chat_id"],
                                                                       "type":"p2p",
                                                                       "name":f"{sender['id']}/{reciver['id']}","photo":None,"about":None,
                                                                       "publickeycrypt":pbc,
...
async def GetUserInfo(request:Request):
    ...
    dt = {"userid":user['id'],
                    ...
                    "photo":user["photo"],
                    "phone":user['phone'],
                    "publickeys":{True:{"publickey":user["publickey"],"publickeycrypt":user["publickeycrypt"]},None:{},False:{}}.get(pbk)
                    }
    print(dt)
    if user["about"]!=None:
...
async def getus(request: Request):
    ...
    if data["photo"]:
        d["photo"]=us["photo"]
    if data["publickey"]:
        d["keys"]={"publickey":us["publickey"],"publickeycrypt":us["publickeycrypt"]}
    return d
